...
| 5 Sequences + replies | Followup delays, reply detection, stop-on-reply, bounce/unsub | decomposed into 5a/5b/5c. 5a reply detection CODE COMPLETE + static-verified (tsc 0, vitest 57, build green); live e2e pending re-consent + reply. 5b followups, 5c bounce/unsub next |
...
Verified end-to-end on Neon: auth (User/Account/Membership OWNER), import (dedupe+suppression), templates/placeholders/campaigns, and Phase 4 mailbox connect + Gmail send (real email delivered to `SEND_OVERRIDE_TO`, `Message` SENT, `CampaignLead` DONE). `tsc` 0, `vitest` 57/57, production `next build` green. (Phase 5a reply detection is static-verified — tsc/vitest/build — but its live reply e2e is still pending.)
